...
- **超限处置**：每一次超限按水库与时刻单列（时刻、水位、限水位、超出多少、判定依据是汛期还是非汛期），逐条登记原因、处置措施、处置时刻与处置人；处置后给出前后水位对照、回落时刻与是否落回限内，登记复核后才算闭环。
...
7. **超限闭环**：超限事件由水位记录按口径 3 自动判定；**回落** = 这次超限之后第一次出现不超限的水位记录；处置状态流转：待处置 → 处置中 → 待复核 → 已闭环（已登记处置 + 水位已回落 + 已复核），已复核但还没回落时显示「已复核待回落」。处置登记跟着水位记录走，水位记录删除时一并删除。
...
| DELETE | /api/levels/:id | 删除一条水位记录（对应的超限处置登记一并删除） |
| GET | /api/exceedances | 超限事件清单（支持 reservoirId、status、from、to；status 可填 未闭环） |
| GET | /api/exceedances/:levelId | 单次超限详情（含回落过程水位记录 trace） |
| PUT | /api/exceedances/:levelId/handling | 登记/修改处置（原因、措施、处置时刻、处置人） |
| PUT | /api/exceedances/:levelId/review | 登记/修改复核（复核人、复核时刻、复核结论；要先登记处置） |
